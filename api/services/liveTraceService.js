...
const getPrimaryInput = (practice) => {
  const parsed = safeJsonParse(practice?.test_cases?.[0]?.input);
  return Array.isArray(parsed) ? parsed : [];
};

const createStep = ({ line, label, detail, state }) => ({ line, label, detail, state });

const buildFallbackTrace = (practice, code) => {
  const inputs = getPrimaryInput(practice);
  return {
    algorithm: 'Guided Replay',
    structure: Array.isArray(inputs[0]) ? 'Array' : 'Execution State',
    code: code || practice?.starter_code || '',
    steps: [
      createStep({
        line: 1,
...
const traceTwoSum = (practice, code = '') => {
  const [nums = [], target] = getPrimaryInput(practice);
  const seen = {};
  const steps = [
    createStep({
      line: 1,
  ...
  for (let i = 0; i < nums.length; i += 1) {
    ...
    if (matchIndex !== undefined) {
      ...
      return {
        ...
        code: code || practice?.solution_code || practice?.starter_code || '',
        steps,
      };
    }
  ...
  return {
    algorithm: 'Two Sum',
    structure: 'Array + Hash Map',
    code: practice?.solution_code || practice?.starter_code || '',
    steps,
  };
};

const traceBinarySearch = (practice, insertMode = false, code = '') => {
  const [nums = [], target] = getPrimaryInput(practice);
  let left = 0;
...
const traceMaxProfit = (practice, code = '') => {
  const [prices = []] = getPrimaryInput(practice);
...
const generateLiveTrace = ({ practice, code = '' }) => {
  if (!practice || String(practice.category || '').toLowerCase() !== 'dsa') {
    return null;
  }

  const title = String(practice.title || '').toLowerCase();
  if (title.includes('two sum')) {
    return traceTwoSum(practice, code);
  }
  if (title === 'binary search') {
    return traceBinarySearch(practice, false, code);
  }
  if (title.includes('search insert')) {
    return traceBinarySearch(practice, true, code);
  }
  if (title.includes('buy and sell stock')) {
    return traceMaxProfit(practice, code);
  }
  return buildFallbackTrace(practice, code);
};

module.exports = { generateLiveTrace };
